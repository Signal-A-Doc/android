...
import com.signalDoc_doctor.ui.fragment.SignupLoginPhase.SignUpFragment
import com.signalDoc_doctor.ui.fragment.WelcomePhase.IntroductionFragment
...
class LoginSignUpActivity : BaseActivity() {
    ...
    override fun onClick(v: View) {
        when (v.id) {
            R.id.backIV -> {
                ...
                if (fragment is SignUpFragment || fragment is CongratulationFragment || fragment is BankDetailFragment) {
                    gotoLoginFragment()
    ...
    override fun onBackPressed() {
        ...
        if (fragment is LoginFragment || fragment is IntroductionFragment) {
            backAction()
        } else if (fragment is SignUpFragment || fragment is CongratulationFragment || fragment is BankDetailFragment) {
            gotoLoginFragment()
        } else if (supportFragmentManager.backStackEntryCount > 0) {
            supportFragmentManager.popBackStack()
        } else {
