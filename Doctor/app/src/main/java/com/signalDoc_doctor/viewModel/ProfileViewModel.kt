...
package com.signalDoc_doctor.viewModel
...
class ProfileViewModel : BaseViewModel() {
    ...
    private var specilizationEditProfileUpdate = SpecilizationEditProfile.getInstance()
    private var selectedDate: Calendar? = null
    private var selectedIssueDate: Calendar? = null
    ...
    private var currentIssueDate: Calendar? = null
    ...
    fun showEditDialog(title: String, type: Int, topTitle: String) {
        ...
        binding = DataBindingUtil.inflate<FragmentEditInfoDialogBinding>(LayoutInflater.from(baseActivity), R.layout.fragment_edit_info_dialog, null, false)
        dialogBuilder.setView(binding!!.root)
        val alertDialog = dialogBuilder.create()
        binding!!.editTV.setText(topTitle)
        binding!!.editHead.setText(topTitle)
        ...
        } else if (type == Const.ISSUE_DATE_PROFILE) {
            binding!!.editET.isFocusable = false
            binding!!.editET.setOnClickListener {
                getCalenderInstance()
                showIssueDatePicker(Const.ISSUE_DATE)
            }


        } else if (type == Const.EXPIRY_DATE_PROFILE) {
            binding!!.editET.isFocusable = false
            binding!!.editET.setOnClickListener {
                getCalenderInstance()
                showIssueDatePicker(Const.EXPIRY_DATE)
        ...
        } else if (type == Const.PROFESSIONAL_STATUS) {
            binding!!.editET.isFocusable = false
            binding!!.editET.setOnClickListener {
                apiHitProfessinal()
                alertDialog.dismiss()
            }
        } else if (type == Const.MEDITATIONS || type == Const.AREA_SPECILIZATION) {
            binding!!.editET.isFocusable = false
            binding!!.editET.setOnClickListener {
                getCategoriesList()
            }
        ...
        if (type == Const.PHONE || type == Const.WEIGHT || type == Const.HEIGHT) {
            binding!!.editET.setInputType(InputType.TYPE_CLASS_NUMBER)
        }
        ...
        binding!!.okBT.setOnClickListener {
            ...
            } else if (type == Const.ISSUE_DATE_PROFILE || type == Const.EXPIRY_DATE_PROFILE) {
                profileUpdate.sendCallBack(SimpleDateFormat("yyyy-MM-dd", Locale.US).format(selectedIssueDate!!.time), type)
            } else if (type == Const.AREA_SPECILIZATION) {
                specilizationEditProfileUpdate.sendCallBack(selectedCats)
            }
            else if (type == Const.COUNTRY) {
    ...
    fun showIssueDatePicker(type: Int) {
        val today = Date()
        ...
        val datePickerDialog = DatePickerDialog(baseActivity, R.style.animateDialog, DatePickerDialog.OnDateSetListener { view, year, monthOfYear, dayOfMonth ->
            selectedIssueDate!!.set(year, monthOfYear, dayOfMonth)
            ...
            binding!!.editET!!.setText(SimpleDateFormat("dd-MM-yyyy").format(selectedIssueDate!!.time))
        }, selectedIssueDate!!.get(Calendar.YEAR), selectedIssueDate!!.get(Calendar.MONTH), selectedIssueDate!!.get(Calendar.DAY_OF_MONTH))
    ...
    fun getCalenderInstance() {
        currentIssueDate = Calendar.getInstance()
        selectedIssueDate = Calendar.getInstance()
        currentIssueDate!!.add(Calendar.MINUTE, 1)
    ...
    override fun onSyncSuccess(responseCode: Int, responseMessage: String, responseUrl: String, response: String?) {
        ...
        try {
            if (responseUrl.equals(Const.API_AREA_OF_SPECILIZATION)) {
    ...
    fun apiHitUpdateWorkProfile(professtionalStatus: String, professionaImunty: String, folioNumber: String, regisNum: String, issueDate: String, expiryNumber: String, summary: String, certificateFile: File?, certificateAnnualFile: File?, selectedCats: ArrayList<CategoriesData>) {
        val params = Api2MultipartByte()
        params.put("Workprofile[professional_status]", professtionalStatus)
        params.put("Workprofile[professional_indemnity]", professionaImunty)
        params.put("Workprofile[MDCN_folio_number]", folioNumber.toInt())
        if (certificateFile != null) {
            params.put("Workprofile[MDCN_certificate]", certificateFile)
        }
        if (certificateAnnualFile != null) {
            params.put("Workprofile[professional_indemnity_certificate]", certificateAnnualFile)
        }
        params.put("Workprofile[registeration_number]", regisNum)
        params.put("Workprofile[issued_date]", baseActivity.changeDateFormat(issueDate, "dd-MM-yyyy", "yyyy-MM-dd"))
        params.put("Workprofile[expiry_date]", baseActivity.changeDateFormat(expiryNumber, "dd-MM-yyyy", "yyyy-MM-dd"))
        params.put("Workprofile[profile_summary]", summary)
        if (!selectedCats.isNullOrEmpty()) {
            var selectedCatIds = ""
            for (i in 0 until selectedCats.size) {
                if (i == 0) {
                    selectedCatIds = selectedCats.get(i).id.toString() + ","
                } else {
                    selectedCatIds = StringBuilder().append(selectedCatIds).append(selectedCats.get(i).id).append(",").toString()
                }
            }
            if (selectedCatIds.length > 0) {
                selectedCatIds = selectedCatIds.substring(0, selectedCatIds.length - 1)
            }
            params.put("Doctorspecialization[categories]", selectedCatIds)
        }
    ...
    private fun showCategoryList() {
        ...
        if (Build.VERSION.SDK_INT > Const.JEALY_BEAM) {
