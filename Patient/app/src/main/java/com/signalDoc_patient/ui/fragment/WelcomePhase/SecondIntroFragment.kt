...
import com.signalDoc_patient.databinding.FragmentWelcomeTwoBinding
...
class SecondIntroFragment : BaseFragment(), ClickHandler {
    private var binding: FragmentWelcomeTwoBinding? = null
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        (baseActivity as LoginSignUpActivity).setToolbar(false)

        if (binding == null) {
            binding = DataBindingUtil.inflate(inflater, R.layout.fragment_welcome_two, container, false)
        }
        return binding!!.root
    }
